...
export class Store {
  ...
  constructor(defaultSelection: () => ModelSelection) {
    ...
    // busy never survives a restart — no turn does either. Persist this
    // normalization once so a later restart cannot rediscover stale state.
    let normalized = false;
    for (const b of this.bots) {
      if (b.busy) normalized = true;
      b.busy = false;
