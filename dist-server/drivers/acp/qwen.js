...
export function ensureQwenInjectModel(modelId, env = process.env) {
    ...
    const envMap = settings.env && typeof settings.env === "object" && !Array.isArray(settings.env)
        ? { ...settings.env }
        : {};
    envMap[keyName] = key;
    settings.env = envMap;
    const providers = settings.modelProviders && typeof settings.modelProviders === "object" && !Array.isArray(settings.modelProviders)
        ? { ...settings.modelProviders }
        : {};
    const openai = Array.isArray(providers.openai) ? [...providers.openai] : [];
    const match = openai.find((row) => row &&
        typeof row === "object" &&
        row.id === inject.model &&
        row.baseUrl === host.baseUrl);
    if (!match) {
        openai.push({
            id: inject.model,
