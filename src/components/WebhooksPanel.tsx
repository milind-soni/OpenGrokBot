...
function relativeTime(at: number | undefined, locale: string, t: Translate) {
  if (!at) return t("Never");
  const elapsed = Math.max(0, Date.now() - at);
  if (elapsed < 60_000) return t("Just now");
  if (elapsed < 60 * 60_000) return t("{count}m ago", { count: Math.floor(elapsed / 60_000) });
  if (elapsed < 24 * 60 * 60_000) return t("{count}h ago", { count: Math.floor(elapsed / 3_600_000) });
...
function suggestedName(prompt: string, bot: Bot | undefined, t: Translate) {
  const first = prompt.trim().split(/[.!?\n]/)[0]?.trim().slice(0, 60);
  return first || t("{name} webhook", { name: bot?.name ?? "MAUS" });
}
...
function WebhookEditor({ webhook, bots, onClose, onCredential }: { webhook?: WebhookTrigger; bots: Bot[]; onClose: () => void; onCredential: (credential: WebhookCredential, webhookId: string) => void }) {
  ...
  const save = async () => {
    ...
    const input: WebhookTriggerInput = { name: name.trim() || suggestedName(prompt, bot, t), prompt: prompt.trim(), botId, runOn, ...webhookActivationDefaults(webhook), eventTypes: eventTypes.split(",").map((value) => value.trim()).filter(Boolean) };
    setSaving(true);
    setError("");
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-5 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="flex max-h-[90vh] w-full max-w-[590px] flex-col overflow-hidden rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="flex items-start justify-between border-b border-hairline/40 px-5 py-4"><div><div className="text-[17px] font-semibold text-ink">{t(webhook ? "Edit webhook" : "New local webhook")}</div><div className="mt-1 text-[12px] text-ink-secondary">{t("Each request starts a new task in the MAUS chat.")}</div></div><button onClick={onClose} className="rounded-lg p-2 text-ink-secondary hover:bg-raised hover:text-ink"><X size={18} /></button></div>
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          <div><div className="mb-2 text-[12px] font-medium text-ink-secondary">{t("Who receives the tasks?")}</div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{bots.map((bot) => <button key={bot.id} type="button" onClick={() => setBotId(bot.id)} className={cn("flex min-w-0 items-center gap-2 rounded-xl border px-3 py-2 text-left", botId === bot.id ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-inset hover:bg-raised/60")}><BotAvatar bot={bot} state={botId === bot.id ? "happy" : "idle"} size={38} animated={false} /><span className="min-w-0 flex-1 truncate text-[13px] font-medium text-ink">{bot.name}</span></button>)}</div></div>
          <div className="rounded-xl border border-accent/20 bg-accent/5 px-3.5 py-3 text-[11.5px] leading-relaxed text-ink-secondary">{t("Send the task in the request:")} <code className="text-ink">{`{"task":"Check the failed build"}`}</code>{t(". The MAUS keeps its model, tools, permissions, and computer setup.")}</div>
          <details className="group rounded-xl border border-hairline/45 bg-inset/45 px-4 py-3" open={Boolean(webhook)}>
            <summary className="cursor-pointer text-[12.5px] font-medium text-ink">{t("Advanced options")}</summary>
            <div className="mt-4 space-y-4">
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">{t("Name")} <span className="font-normal">· {t("optional")}</span></span><input value={name} onChange={(event) => setName(event.target.value)} placeholder={suggestedName(prompt, bots.find((bot) => bot.id === botId), t)} className="w-full rounded-xl border border-hairline/60 bg-panel px-3.5 py-2.5 text-[13px] text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /></label>
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">{t("Default instructions")} <span className="font-normal">· {t("optional")}</span></span><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={3} placeholder={t("For every event, summarize what happened and suggest the next step…")} className="w-full resize-y rounded-xl border border-hairline/60 bg-panel px-3.5 py-3 text-[13px] leading-relaxed text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /><span className="mt-1.5 block text-[10.5px] leading-relaxed text-ink-secondary">{t("Use this only when every event needs the same handling rule. Otherwise the request’s task is used.")}</span></label>
              <div><div className="mb-2 text-[11.5px] font-medium text-ink-secondary">{t("Run on")}</div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setRunOn("maus")} className={cn("rounded-xl border p-3 text-left", runOn === "maus" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Laptop size={14} />{t("This computer")}</div></button><button type="button" disabled={!cloudReady && runOn !== "cloud"} onClick={() => setRunOn("cloud")} className={cn("rounded-xl border p-3 text-left disabled:cursor-not-allowed disabled:opacity-45", runOn === "cloud" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Cloud size={14} />{t("Cloud VM")}</div></button></div></div>
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">{t("Only accept event types")} <span className="font-normal">· {t("optional")}</span></span><input value={eventTypes} onChange={(event) => setEventTypes(event.target.value)} placeholder="push, workflow_run" className="w-full rounded-xl border border-hairline/60 bg-panel px-3.5 py-2.5 text-[13px] text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /><span className="mt-1.5 block text-[10.5px] text-ink-secondary">{t("Comma-separated values from the sender’s event-type header.")}</span></label>
            </div>
          </details>
          {error && <div className="rounded-xl border border-danger/30 bg-danger/10 px-3.5 py-3 text-[12px] text-danger">{error}</div>}
        </div>
        <div className="flex justify-end gap-2 border-t border-hairline/40 px-5 py-4"><button onClick={onClose} className="rounded-xl px-4 py-2 text-[13px] text-ink-secondary hover:bg-raised hover:text-ink">{t("Cancel")}</button><button disabled={saving || !botId} onClick={() => void save()} className="flex items-center gap-2 rounded-xl bg-accent px-4 py-2 text-[13px] font-medium text-white hover:brightness-110 disabled:opacity-40">{saving && <Loader2 size={14} className="animate-spin" />}{t(webhook ? "Save changes" : "Create local webhook")}</button></div>
      </div>
    </div>
  );
}
...
export function WebhooksPanel({ bots }: { bots: Bot[] }) {
  ...
  const activity = useMemo<ActivityItem[]>(() => {
    ...
    const attempts = state.webhookAttempts
      ...
      .map((attempt) => ({
        ...
        eventName: attempt.eventName || t(attempt.outcome === "rejected" ? "Rejected request" : "Webhook event"),
  ...
  }, [attemptsByRun, runById, selected, state.routineRuns, state.webhookAttempts, t]);
  ...
  const invoke = async (webhook: WebhookTrigger, action: "toggle" | "delete") => {
    ...
    try {
      if (action === "delete") {
        if (!window.confirm(t("Delete “{name}”? Existing task history will stay available.", { name: webhook.name }))) return;
        ...
        if (enabling && webhook.verificationPending) throw new Error(t("Send a request before turning this webhook on"));
  ...
  const createAndCopyCommand = async (webhook: WebhookTrigger, replace = false) => {
    if (replace && !window.confirm(t("Replace this private URL? Every previously copied command will stop working."))) return;
    ...
    try {
      ...
      if (!credential) throw new Error(t("Could not create a terminal command"));
      ...
      <div className="mx-auto max-w-[1120px] space-y-4">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-[17px] font-semibold text-ink">{t("Webhooks")}</h2>
              <span className="rounded-md bg-accent/10 px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wider text-accent">{t("Local beta")}</span>
            </div>
            <p className="mt-1 text-[12px] text-ink-secondary">{t("Send a task to a MAUS when another tool reports an event.")}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className={cn("hidden items-center gap-1.5 text-[10.5px] sm:flex", ingress?.available ? "text-success" : "text-danger")}>
              ...
              {ingress?.available ? t("Receiver running") : ingress?.error ?? t("Receiver unavailable")}
            </div>
        ...
        {state.webhooks.length === 0 ? (
          <div className="border-t border-hairline/40 px-6 py-16 text-center">
            ...
            <h3 className="text-[16px] font-semibold text-ink">{t("Create your first webhook")}</h3>
            <p className="mx-auto mt-2 max-w-[420px] text-[12.5px] leading-relaxed text-ink-secondary">{t("Choose a MAUS, copy one command, and every request becomes a new task in its chat.")}</p>
            <button onClick={() => setEditor("new")} disabled={bots.length === 0} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-[13px] font-medium text-white hover:brightness-110 disabled:opacity-40"><Plus size={15} />{t("Create local webhook")}</button>
            {bots.length === 0 && <p className="mt-3 text-[12px] text-warning">{t("Create a MAUS first, then come back here.")}</p>}
          </div>
        ) : selected && (
          <div className="min-h-[580px] overflow-hidden rounded-2xl border border-hairline/45 bg-panel/25 md:grid md:grid-cols-[250px_minmax(0,1fr)]">
            <aside className="border-b border-hairline/40 bg-inset/35 p-2.5 md:border-r md:border-b-0">
              <div className="flex items-center justify-between px-2 py-2">
                <span className="text-[10px] font-medium uppercase tracking-[0.12em] text-ink-secondary">{t("Your webhooks")}</span>
                  ...
                  return (
                    <button key={webhook.id} onClick={() => { setSelectedId(webhook.id); setTab("setup"); setError(""); }} className={cn("flex min-w-[210px] items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left transition-colors md:w-full md:min-w-0", webhook.id === selected.id ? "bg-raised text-ink shadow-sm" : "text-ink-secondary hover:bg-raised/55 hover:text-ink")}>
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-0.5 flex items-center gap-1.5 text-[10px]"><span className={cn("size-1.5 rounded-full", status.dot)} /><span className={status.tone}>{t(status.label)}</span></span>
                      </span>
                    </button>
                  );
                })}
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-5 md:px-7 md:pt-6">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10.5px] text-ink-secondary"><span>{selectedBot?.name ?? t("Deleted MAUS")}</span><span>·</span><span>{t(selected.runOn === "cloud" ? "Cloud VM" : "This computer")}</span></div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {!selected.verificationPending && <button disabled={Boolean(working)} onClick={() => void invoke(selected, "toggle")} className={cn("flex items-center gap-1.5 rounded-lg px-3 py-2 text-[11.5px] font-medium", selected.enabled ? "bg-raised text-ink-secondary hover:text-ink" : "bg-accent text-white hover:brightness-110")}>{selected.enabled ? <Pause size={13} /> : <Play size={13} />}{t(selected.enabled ? "Pause" : "Enable")}</button>}
                  <details className="relative"><summary className="list-none rounded-lg p-2 text-ink-secondary hover:bg-raised hover:text-ink"><MoreHorizontal size={17} /></summary><div className="absolute right-0 top-full z-20 mt-1 w-[180px] rounded-xl border border-hairline/50 bg-card p-1.5 shadow-2xl"><button onClick={() => setEditor(selected)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[12px] text-ink hover:bg-raised">{t("Edit settings")}</button><button onClick={() => void invoke(selected, "delete")} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[12px] text-danger hover:bg-danger/10"><Trash2 size={13} />{t("Delete")}</button></div></details>
                </div>
              </div>

              <div className="mt-5 flex gap-5 border-b border-hairline/35 px-5 md:px-7">
                <button onClick={() => setTab("setup")} className={cn("border-b-2 px-0.5 pb-3 text-[12px] font-medium", tab === "setup" ? "border-accent text-ink" : "border-transparent text-ink-secondary hover:text-ink")}>{t("Setup")}</button>
                <button onClick={() => setTab("activity")} className={cn("flex items-center gap-1.5 border-b-2 px-0.5 pb-3 text-[12px] font-medium", tab === "activity" ? "border-accent text-ink" : "border-transparent text-ink-secondary hover:text-ink")}>{t("Activity")}{activity.length > 0 && <span className="rounded-full bg-raised px-1.5 py-0.5 text-[9px] tabular-nums text-ink-secondary">{activity.length}</span>}</button>
              </div>

              {tab === "setup" ? (
                <div className="px-5 py-6 md:px-7">
                  <div className="max-w-[720px]">
                    <h4 className="text-[14px] font-semibold text-ink">{t("Send a task")}</h4>
                    <p className="mt-1 text-[11.5px] leading-relaxed text-ink-secondary">{t("Copy this command into Terminal and press Return. It starts a real task in {name}'s chat; edit the task text for whatever you want done.", { name: selectedBot?.name ?? t("this MAUS") })}</p>
                    {credential ? (
                      <div className="mt-4 overflow-hidden rounded-xl border border-hairline/45 bg-[#0d0d0d]">
                        <div className="flex items-center justify-between border-b border-white/5 px-3.5 py-2"><span className="text-[9.5px] font-medium uppercase tracking-wider text-ink-secondary">{t("Terminal")}</span><div className="flex items-center gap-1"><button onClick={() => void createAndCopyCommand(selected)} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10.5px] font-medium text-ink-secondary hover:bg-raised hover:text-ink">{copiedId === selected.id ? <Check size={12} className="text-success" /> : <Copy size={12} />}{t(copiedId === selected.id ? "Copied" : "Copy command")}</button><button onClick={() => void createAndCopyCommand(selected, true)} className="rounded-lg p-1.5 text-ink-secondary hover:bg-raised hover:text-ink" title={t("Rotate private URL")}><RotateCw size={12} /></button></div></div>
                      ...
                      <div className="mt-4">
                        <p className="max-w-[560px] text-[10.5px] leading-relaxed text-ink-secondary">{t("The private URL is shown once. Generate a replacement to copy your webhook command; any older command for this webhook will stop working.")}</p>
                        <button disabled={Boolean(working) || !ingress?.available} onClick={() => void createAndCopyCommand(selected)} className="mt-3 flex items-center gap-2 rounded-xl bg-accent px-3.5 py-2.5 text-[12px] font-medium text-white hover:brightness-110 disabled:opacity-40">{working === `${selected.id}:command` ? <Loader2 size={14} className="animate-spin" /> : <RotateCw size={14} />}{t("Generate new private URL")}</button>
                      </div>
                    )}
                    <div className="mt-3 flex items-start gap-2 text-[10.5px] leading-relaxed text-ink-secondary"><Laptop size={12} className="mt-0.5 shrink-0" /><span>{t("Local only for now. Keep OpenMausBot open while sending the request.")}</span></div>

                    {selected.verificationSample && !selected.enabled && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-y border-success/20 bg-success/5 px-3.5 py-3"><div><div className="flex items-center gap-2 text-[11.5px] font-medium text-success"><Check size={13} />{t("Request received")}</div><p className="mt-1 max-w-[520px] truncate font-mono text-[10px] text-ink-secondary">{selected.verificationSample.preview || t("Empty payload")}</p></div><button disabled={Boolean(working)} onClick={() => void invoke(selected, "toggle")} className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-[11px] font-medium text-white hover:brightness-110"><Play size={12} />{t("Turn on")}</button></div>}

                    <div className="mt-7 border-t border-hairline/35 pt-5">
                      <div className="grid gap-4 text-[11.5px] sm:grid-cols-2"><div><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("Tasks go to")}</div><div className="mt-1.5 font-medium text-ink">{selectedBot?.name ?? t("Deleted MAUS")}</div></div><div><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("Runs on")}</div><div className="mt-1.5 font-medium text-ink">{t(selected.runOn === "cloud" ? "Cloud VM" : "This computer")}</div></div></div>
                    </div>

                    <details className="group mt-5 border-t border-hairline/35 pt-4"><summary className="flex cursor-pointer list-none items-center justify-between text-[11.5px] font-medium text-ink"><span>{t("Advanced")}</span><ChevronDown size={14} className="text-ink-secondary transition-transform group-open:rotate-180" /></summary><div className="mt-4 space-y-3 text-[10.5px] leading-relaxed text-ink-secondary">{selected.prompt ? <p><span className="font-medium text-ink">{t("Default instruction:")}</span> {selected.prompt}</p> : <p>{t("The task or message sent with each request becomes the MAUS instruction.")}</p>}{selected.eventTypes?.length ? <p><span className="font-medium text-ink">{t("Accepted events:")}</span> {selected.eventTypes.join(", ")}</p> : <p>{t("All event types are accepted.")}</p>}<button onClick={() => setEditor(selected)} className="rounded-lg border border-hairline/50 px-3 py-2 text-[11px] font-medium text-ink hover:bg-raised">{t("Edit settings")}</button></div></details>
                  </div>
                </div>
              ) : (
                <div className="px-5 py-5 md:px-7">
                  <div className="mb-3"><h4 className="text-[13px] font-semibold text-ink">{t("Recent deliveries")}</h4><p className="mt-1 text-[10.5px] text-ink-secondary">{t("Accepted and rejected requests update automatically.")}</p></div>
                  <div className="border-t border-hairline/35">
                    {activity.length === 0 ? <div className="px-2 py-12 text-center text-[11.5px] text-ink-secondary">{t("No requests yet. Use the command in Setup to send one.")}</div> : activity.map((item) => (
