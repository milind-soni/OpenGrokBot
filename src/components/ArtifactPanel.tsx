...
import { Check, Copy, Download, RefreshCw, Wifi, X } from "lucide-react";
...
export function ArtifactPanel({
  ...
  const startResize = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const move = (moveEvent: PointerEvent) => onWidthChange(clamp(startWidth - (moveEvent.clientX - startX)));
    const end = () => {
      ...
      <div className="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-hairline/40 px-3">
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <span
            ...
            title="External resources may connect to the network"
            aria-label="External resources may connect to the network"
          >
            <Wifi size={14} />
          </span>
          <button onClick={() => setRevision((value) => value + 1)} className="rounded-md p-1.5 text-ink-secondary hover:bg-raised hover:text-ink" title="Refresh preview">
            <RefreshCw size={15} />
          </button>
          <button onClick={() => void copy()} className="rounded-md p-1.5 text-ink-secondary hover:bg-raised hover:text-ink" title="Copy HTML">
            {copied ? <Check size={15} className="text-success" /> : <Copy size={15} />}
          </button>
          <button onClick={download} className="rounded-md p-1.5 text-ink-secondary hover:bg-raised hover:text-ink" title="Download HTML">
            <Download size={15} />
          </button>
          <button onClick={onClose} className="rounded-md p-1.5 text-ink-secondary hover:bg-raised hover:text-ink" title="Close preview">
            <X size={16} />
          </button>
        </div>
      </div>
