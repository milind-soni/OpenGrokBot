...
import { closeSync, existsSync, mkdirSync, openSync, readSync, readdirSync, renameSync, statSync, unlinkSync, writeFileSync, writeSync, } from "node:fs";
...
const MIME_BY_EXTENSION = Object.fromEntries(Object.entries(EXTENSION_BY_MIME).map(([mime, extension]) => [extension, mime]));
...
export function createMediaCache(options) {
    ...
    const finalize = (partialPath, claimedMime, kind, bytes) => {
        ...
        if (!actual || expectedKind(actual) !== kind)
            throw new Error(`generated ${kind} has an unsupported file signature`);
        ...
        if (!extension)
            throw new Error(`generated media type ${actual} is not supported`);
    ...
    const writeBase64 = (source, context) => {
        ...
        if (data.length > limit)
            throw new Error(`generated ${context.kind} exceeds the ${limit} byte cache limit`);
    ...
    const writeRemote = async (source, context) => {
        ...
        if (!response?.ok || !response.body)
            throw new Error(`generated media download failed with HTTP ${response?.status ?? 0}`);
        const limit = context.kind === "image"
            ? (options.imageLimitBytes ?? DEFAULT_IMAGE_LIMIT)
            : (options.videoLimitBytes ?? DEFAULT_VIDEO_LIMIT);
    ...
    return {
        store: (source, context) => source.type === "base64" ? Promise.resolve().then(() => writeBase64(source, context)) : writeRemote(source, context),
        resolve: (cacheKey) => {
            if (basename(cacheKey) !== cacheKey || !/^[0-9a-f-]+\.(png|jpg|gif|webp|mp4|webm)$/.test(cacheKey))
                return null;
            const path = resolve(objectsDir, cacheKey);
            if (!path.startsWith(resolve(objectsDir) + sep) || !existsSync(path))
                return null;
