...
**Approach:**
- In `settle()`, call `killCliTree(child)` unconditionally, before or alongside the existing `broker?.close()` and temp-dir cleanup. `child` is already in scope (defined earlier in `sendTurn` at the `spawnCli` call); do not introduce a new binding or rely on the later-declared `stop` const.
