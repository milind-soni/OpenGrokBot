...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (method === "GET" && path === "/api/templates") {
      return json(res, 200, { items: (cfg.templates?.items ?? []).map(templateSnapshot) });
    }
    if (method === "POST" && path === "/api/templates") {
      try {
        const item = normalizeTemplate(await readBody(req));
        saveConfig({ templates: { items: [...(cfg.templates?.items ?? []), item] } });
        Object.assign(cfg, loadConfig());
        broadcast({ kind: "config", ...configStatus() });
        return json(res, 201, { item: templateSnapshot(item) });
      } catch (error) {
        return json(res, 400, { error: error instanceof Error ? error.message : "Invalid template" });
      }
    }
    m = path.match(/^\/api\/templates\/([\w-]+)$/);
    if (m && method === "DELETE") {
      const items = cfg.templates?.items ?? [];
      if (!items.some((item) => item.id === m![1])) return json(res, 404, { error: "no such template" });
      saveConfig({ templates: { items: items.filter((item) => item.id !== m![1]) } });
      Object.assign(cfg, loadConfig());
      broadcast({ kind: "config", ...configStatus() });
      return json(res, 200, { ok: true });
    }
